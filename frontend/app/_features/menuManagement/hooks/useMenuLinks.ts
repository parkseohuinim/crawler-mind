...
import { MenuLink, MenuLinkCreate, MenuLinkUpdate, menuLinkService } from '@/app/_lib/domains/menuLink';
...
export function useMenuLinks() {
  ...
  const fetchMenuLinks = useCallback(async (page?: number, size?: number, search?: string) => {
    ...
    try {
      ...
      const response = await menuLinkService.getMenuLinks(actualPage, actualSize, actualSearch || undefined);
  ...
  const createMenuLink = useCallback(async (menuLink: MenuLinkCreate): Promise<boolean> => {
    ...
    try {
      await menuLinkService.createMenuLink(menuLink);
  ...
  const updateMenuLink = useCallback(async (id: number, menuLink: MenuLinkUpdate): Promise<boolean> => {
    ...
    try {
      await menuLinkService.updateMenuLink(id, menuLink);
  ...
  // Get available menu links for manager assignment
  const getAvailableMenuLinksForManager = useCallback(async (page?: number, size?: number, search?: string) => {
    try {
      const actualPage = page ?? 1;
      const actualSize = size ?? 100;
      const actualSearch = search;
      
      return await menuLinkService.getAvailableMenuLinksForManager(actualPage, actualSize, actualSearch);
    } catch (error) {
      console.error('Failed to get available menu links for manager:', error);
      throw error;
    }
  }, []);
