  ...
  const generateCollapsibleTree = (menuLinks: MenuLink[]): TreeNode => {
    ...
    menuLinks.forEach(menuLink => {
      ...
      pathParts.forEach((part, index) => {
        ...
        // Add menu link to the final node (leaf)
  ...
  return (
    <div className="menu-tree-chart">
          ...
          <div className="details-card">
            ...
            {selectedNode.menuLinks.length > 0 && (
              <div className="details-menus">
                <h5>연결된 메뉴 링크 ({selectedNode.menuLinks.length}개)</h5>
                <div className="menu-list">
                  {selectedNode.menuLinks.map((menu, index) => (
      ...
      <div className="chart-legend" style={{
        ...
        <h4 style={{ margin: '0 0 12px 0', color: '#2d3748', fontSize: '1rem', fontWeight: '600' }}>
          레벨별 색상 범례
        </h4>
          ...
          {legendData.map((item) => (
            <div key={item.level} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: '600', color: '#2d3748', fontSize: '0.8rem' }}>
                  Level {item.level}: {item.label}
                </div>
                <div style={{ color: '#718096', fontSize: '0.75rem', lineHeight: '1.2' }}>
                  {item.description}
                </div>
              </div>
            </div>
        ...
        <div style={{ 
          ...
          <div>표시 중: {allNodes.nodes.length}개 노드</div>
          <div>전체: {(() => {
