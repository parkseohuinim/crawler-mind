...
class DailyCrawlingService:
    ...
    async def _crawl_single_url(self, input_url: InputUrl, timeout: int = 300) -> Dict[str, Any]:
        ...
        if handler_info:
            ...
            if "_main" in handler_func.__name__:
                # 다중 결과 핸들러: 개별 상세 페이지에 자체 타임아웃이 있으므로 전체 타임아웃 미적용
                skip_timeout = True
                logger.info(f"🔗 Multi-result handler, no global timeout: {handler_func.__name__}")
            else:
                # 일반 핸들러: 3분(180초) 타임아웃
                timeout = 180
