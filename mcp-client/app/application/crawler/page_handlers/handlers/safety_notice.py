"""
안전한 통신생활 공지사항 관련 핸들러

안전한 통신생활 공지사항 목록 및 상세 페이지 처리
"""
...
from ..utils import sanitize_filename, format_content, create_markdown, smart_goto, launch_chromium

logger = logging.getLogger(__name__)


async def handle_safety_notice_detail(
    url: str, 
    fclient: Any, 
    cutoff_date: Optional[datetime] = None
) -> Dict[str, Any]:
    """
    안전한 통신생활 공지사항 개별 게시물 처리 핸들러
    """
    logger.info(f"🔗 Safety notice detail: {url}")
    ...
    try:
        ...
        logger.error(f"❌ Safety notice failed: {str(e)}")
    ...
    logger.info(f"✅ Safety notice done: '{metadata['title']}'")
...
async def handle_safety_notice_main(
    url: str, 
    fclient: Any, 
    menu: Optional[str] = None
) -> Dict[str, Any]:
    """
    안전한 통신생활 공지사항 메인 목록 페이지 처리
    """
    logger.info(f"🔗 Safety notice main: {url}")
    cutoff_date = datetime.now() - timedelta(days=365)
        ...
        try:
            ...
            try:
                result = await asyncio.wait_for(
                    handle_safety_notice_detail(current_url, fclient, cutoff_date),
                    timeout=120
                )
                consecutive_errors = 0
    ...
    logger.info(f"✅ Safety notice done: {total_processed} items")
